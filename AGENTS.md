...
- ✅ Web 端 Markdown 引擎：`apps/web/src/lib/markdown.ts`（markdown-it 14 + KaTeX 0.16，`html:false` 防注入）——统一 Obsidian 语法支持：frontmatter 与 `%%注释%%` 剥离、`> [!type]` callout（blockquote 挂类型 class 着色 + emoji 标签）、脚注定义降级 + `[^n]` 上标、`==高亮==`、`#标签`、`[[双链]]`、`![[嵌入]]` 图标降级、`$...$`/`$$...$$`/`\(...\)`/`\[...\]` 公式、GFM 任务列表 checkbox、自研轻量代码高亮（5 语言族，零依赖）、标题锚点 id（slug 保留中文、重复自动加序号，`extractHeadings` 与渲染同源保证锚点不失效）。已接入 `/api/export`（PDF 打印 HTML + Anki 卡片背面，替代此前仅支持 6 种语法且全量 escape 的自研 mdToHtml）；导出层另有自动目录（≥3 标题）、打印断页优化（标题不与正文分离、块级元素不跨页）、外链附 URL 便于纸质溯源；公式走 KaTeX `output:'mathml'`（产物自包含，无需 katex.min.css 与字体，katex 与 mermaid 传递依赖同版本复用）
...
- ✅ AI 流式对话 + 工具闭环：`lib/llm.ts` chatWithLlmStream（XHR progress 增量文本读 SSE——SDK 51 无 expo/fetch 且 RN fetch polyfill 不暴露 res.body，tool_calls 按 index 增量累积；自定义中转站 400/404/422 自动降级非流式）；aiStore.ask 多轮工具循环（读工具结果以 role:tool 回传模型综合，写操作仍走确认卡片）；动态系统提示词注入日期/高考倒计时/目标/今日待办；流式上屏 80ms 节流（makeStreamingUpdater）；发送键生成中变红色停止键（AbortController 保留部分文本）；Web 端 llm.ts fetchWithRetry（429/5xx 指数退避 ×2，chat 120s / embedding 30s 超时）
- ✅ AI 语音对话：`lib/tts.ts`（OpenAI 兼容 /audio/speech → mp3 落缓存 → expo-av 播放；speakableText 清洗 Markdown + 600 字朗读上限）+ aiStore 语音状态机（voiceMode/voiceState：录音 60s 兜底 → STT 转写即发送 → 回复自动播报 → 播完自动聆听闭环；speakGen 代数防打断竞态；barge-in：说话时点麦克风/发消息即打断播报）；STT 复用 stt.ts（留空回退 LLM 配置）；AiOrb 麦克风键/耳机键语音模式开关/气泡「播报」重播键（TTS 未配置时自动隐藏）；profile 新增 TTS 配置区（baseUrl/Key/模型/音色，默认 tts-1+alloy）
- ✅ 心流横屏模式：expo-screen-orientation ~7.0.5（SDK 51 配套）。enterFlow unlockAsync（跟随系统自动旋转）/ exitFlow lockAsync(PORTRAIT_UP) 强制回竖——横屏仅存在于心流 Modal 内，App 其余界面保持 app.json portrait 锁；竖屏=原纵排（徽章/68px 计时/氛围句/免打扰/结束），横屏=左计时（84px 独占左半屏）右控制列分栏，左右安全区让开挖孔；「横屏专注/回到竖屏」按钮解决系统自动旋转关闭的用户（lockAsync(LANDSCAPE/PORTRAIT_UP) 强切）；所有方向 API 失败静默降级纯竖屏
