...
- ✅ AI 流式对话 + 工具闭环：`lib/llm.ts` chatWithLlmStream（expo/fetch 原生流读 SSE，tool_calls 按 index 增量累积；自定义中转站 400/404/422 自动降级非流式）；aiStore.ask 多轮工具循环（读工具结果以 role:tool 回传模型综合，写操作仍走确认卡片）；动态系统提示词注入日期/高考倒计时/目标/今日待办；流式上屏 80ms 节流（makeStreamingUpdater）；发送键生成中变红色停止键（AbortController 保留部分文本）；Web 端 llm.ts fetchWithRetry（429/5xx 指数退避 ×2，chat 120s / embedding 30s 超时）
