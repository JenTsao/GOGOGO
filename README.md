...
- **知识工坊**：GitHub 文件树 + Monaco 只读编辑器（保存回写/传图/版本控制见 TODO）
- **语义检索中心**：关键词 `ilike` + pgvector 向量混合检索（HNSW），内容哈希增量向量化
- **编译与输出**：资源池（笔记 + 错题）→ 纯文本大纲 / Anki TSV / 复习 PDF（A4 打印视图），历史最近 10 次
...
- ✅ **Phase 3 专业化与后台**：L4 六大工具 + 确认卡片；编译输出（大纲/Anki TSV/PDF）；六维画像 + Tavily 横向对标；后台唤醒 + 本地提醒通知
- ✅ **Phase 4 错题本**：拍照/压缩/标签/语音反思入库；云同步代理；AI 识别题面 + 读图讲解 + 语音转文字；画像危险学科/卡壳词云/情绪信号全闭环

## TODO（对照蓝皮书未完成项）

**管理台**
- [ ] 知识工坊：编辑保存回写 GitHub、拖拽传图 WebP 压缩
- [ ] 知识工坊：AI 精炼工具栏（勾选多篇 → 🤖 合并精炼 / 🧠 生成知识图谱 Mermaid）
- [ ] 知识工坊：版本控制（`obsidian_metadata.version_history` 快照 + 一键回滚 7 天前）
- [ ] 语义检索中心：层级标签树管理（拖拽重命名/合并/批量关联笔记）
- [ ] 编译输出：真 `.apkg`（现 TSV 导入格式）、代码片段资源接入、错题图片进 PDF
